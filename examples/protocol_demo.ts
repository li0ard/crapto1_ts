import { Crypto1State, oddParity8, prng_successor } from "../src";
...
const append_crc16_a = (buf: number[]): number[] => {
    let crc = 0x6363;
    for (let i = 0; i < 2; i++) {
        crc ^= buf[i];
        for (let j = 0; j < 8; j++) {
...
let tag_ks0 = tag_state.word(tag_uid ^ tag_nt);
...
process.stdout.write("ks0 = crypto1_word(s, uid ^ nT, 0)                                        ")
let reader_ks0 = reader_state.word(reader_uid ^ reader_nt);
...
let reader_ks1 = reader_state.word(reader_nr);
...
let reader_ks2 = reader_state.word(0);
...
let tag_ks1 = tag_state.word(tag_nr_enc, true);
...
let reader_ks3 = reader_state.word(0);
...
console.log("Reader                          <>  Tag\n======                          <>  ===")
let reader_ks4 = reader_state.word(0);
...
let tag2_ks0 = tag2_state.word(tag_uid ^ tag2_nt);
...
let reader2_ks0 = reader2_state.word(reader_uid ^ reader2_nt_enc, true);
...
let reader2_ks1 = reader2_state.word(reader2_nr);
...
let reader2_ks2 = reader2_state.word(0);
...
let tag2_ks1 = tag2_state.word(tag2_nr_enc, true);
...
let tag2_ks2 = tag2_state.word(0);
...
let tag2_ks3 = tag2_state.word(0);
console.log(`                                    ks3 = crypto1_word(s, 0, 0)           ${dec2hex(tag2_ks3, 32)}`)

let tag2_ks4 = tag2_state.word(0);
...
let reader2_ks3 = reader2_state.word(0);
